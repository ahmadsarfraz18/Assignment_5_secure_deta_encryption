...
st.caption("Generated with 🖤 by Mahar Ahmad Sarfraz")
    ...
    if st.button("Login"):
        if username in stored_data and stored_data[username]['password'] == hash_password(password):
            ...
            if st.session_state.failed_attempts >= 3:
                ...
                st.error(f'🛑 Too many failed attempts. You are locked out for {LOCK_DURATION} seconds.')
                st.stop()
    ...
    if not st.session_state.authenticated_user:
        ...
        if not user_data:
            ...
            if st.button('Decrypt'):
                ...
                if result and result != "Invalid key or corrupted data":
